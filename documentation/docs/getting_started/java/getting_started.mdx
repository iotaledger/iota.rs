...
- libudev is needed (for Linux `apt install libudev-dev`)
...
After running one of these commands, the jar can then be found at `iota.rs/bindings/java/native/build/libs/native.jar` or at `iota.rs/bindings/java/iota-client-java/build/libs/iota-client-java-1.0-SNAPSHOT.jar`.
