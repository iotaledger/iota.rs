...
An [`IndexationPayload`](../key_concepts/messages_payloads_and_transactions.md#indexationpayload) is a payload type that can be used to attach arbitrary `data`
...
- There are three prepared [payloads](../key_concepts/messages_payloads_and_transactions.md#corepayloads)(`transaction`, `milestone`, and `indexation`). However, at the time, the `indexation`
...
- IOTA addresses are part of the `UTXO` data structure that is sent using the [`SignedTransaction`](./../key_concepts/messages_payloads_and_transactions#signedtransaction)
  payload.
