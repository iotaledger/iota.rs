...
Once a [message](../../../explanations/messages_payloads_and_transactions.md#messages) is broadcast, you can use the
[`MessageGetter`](./../libraries/wasm/api_reference#messagegetter) helper class that you can instantiate using the
[`Client.getMessage()`](./../libraries/wasm/api_reference#clientgetmessage--codemessagegettercode) function to retrieve
it from the network. The `MessageGetter` provides helper functions related to the message, such as
[`Client.getMessage().data(message_id: String)`](./../libraries/wasm/api_reference#messagegetterdatamessage_id--promiseany)
and
[`Client.getMessage().metadata(message_id: String)`](./../libraries/wasm/api_reference#messagegettermetadatamessage_id--promiseany):
