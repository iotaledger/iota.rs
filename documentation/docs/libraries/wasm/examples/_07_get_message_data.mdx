...
[`MessageGetter`](./../api_reference.md#messagegetter) helper class that you can instantiate using the
[`Client.getMessage()`](./../api_reference.md#clientgetmessage--codemessagegettercode) function to retrieve
it from the network. The `MessageGetter` provides helper functions related to the message, such as
[`Client.getMessage().data(message_id: String)`](./../api_reference.md#messagegetterdatamessage_id--promiseany)
and
[`Client.getMessage().metadata(message_id: String)`](./../api_reference.md#messagegettermetadatamessage_id--promiseany):
