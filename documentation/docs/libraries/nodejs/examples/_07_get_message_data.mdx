...
[`MessageFinder`](./../libraries/nodejs/api_reference#messagefinder) helper class that you can instantiate using the
[`Client.getMessage()`](./../libraries/nodejs/api_reference#getmessage-messagefinder) function to retrieve it from the
network. The `MessageFinder` provides helper functions related to the message, such as [`Client.getMessage().data(id: str)`](./../libraries/nodejs/api_reference#dataid-promise)
and [`Client.getMessage().metadata(str)`](./../libraries/nodejs/api_reference#metadataid-promise):
