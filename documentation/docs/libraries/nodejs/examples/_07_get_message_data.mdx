...
[`MessageFinder`](./../api_reference.md#messagefinder) helper class that you can instantiate using the
[`Client.getMessage()`](./../api_reference.md#getmessage-messagefinder) function to retrieve it from the
network. The `MessageFinder` provides helper functions related to the message, such as [`Client.getMessage().data(id: str)`](./../api_reference.md#dataid-promise)
and [`Client.getMessage().metadata(str)`](./../api_reference.md#metadataid-promise):
