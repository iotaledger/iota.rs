...
You can use the function [`Client.postMessage(message)`](./../libraries/nodejs/api_reference#postmessagemessage-promise) to send a message instance over a network. Alternatively, you can
use the [`MessageSender`](./../libraries/nodejs/api_reference#messagesender) helper class with chaining calls that prepares a message instance and broadcasts it over the
network.
