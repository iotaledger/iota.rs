...
```json
{
   ...
   "url":"https://api.hornet-0.devnet.chrysalis2.com"
}
```
...
* `bech32_hrp`: it indicates whether the given node is a part of devnet (`atoi`) or mainnet (`iota`). See more info regarding [IOTA address format](https://wiki.iota.org/chrysalis-docs/guides/developer/#iota-15-address-anatomy)
...
* `network`: can be `Testnet` or `Mainnet`. It instructs the library whether to automatically select devnet nodes or mainnet nodes
...
## Checking a balance
_In IOTA devnet, there is a faucet service that provides test tokens to any devnet address: https://faucet.chrysalis-devnet.iota.cafe/_
...
Before we continue, let's introduce some additional terms that describe a unit that is actually broadcast in IOTA network. IOTA is based on a concept of `messages` and `payloads`.

`Message` is a data structure that is actually being broadcast in IOTA network and represent a node (vertex) in the Tangle graph. It can refer to up to 8 previous messages and once a message was attached to the Tangle and approved by a milestone, the Tangle structure ensures the content of the message is unaltered. Every message is referenced by `message_id` which is based on a hash algorithm of binary content of the message. `Message` is an atomic unit that is confirmed by network as a whole.

`Message` is broadcast using a binary format, is arbitrary size (up to 35 kB) and it can hold a variable sets of information so called `payloads`. Number of payloads a single message can encapsulate is not given (even a message without any `payload` at all is completely valid).
...
Before the chapter is wrapped up, one thing was left unexplained: _"how outputs are being sent and broadcast to network?"_ `Outputs` are being sent encapsulated in a `message` as a part of `SignedTransaction` payload.
...
As mentioned above, the `message` is encapsulating data structure that is being actually broadcast across network. It is an atomic unit that is accepted/rejected as a whole.

There is a convenient function `Client.message()` that prepares a message instance and sends it over a network. It accepts wide range of input parameters and can help with any kind of message type to be broadcast.
...
Once a message is broadcast, there are two main functions that can be used to read all information about the given message from the Tangle (`Client.get_message_data()` and `Client.get_message_metadata()`):
...
* `index` was simply encoded to `list[bytes]` in hex (no hash algorithm) and the resulting string can be leveraged as an additional way how to search for a set of indexed messages with the same key index via [Tangle explorer](https://explorer.iota.org/devnet/indexed/736f6d655f646174615f696e646578) or `Client.find_messages()` API call
