...
use bee_ternary::{b1t6, T1B1Buf, Trits, TryteBuf};
...
/// Decode Ed25519Address to migration tryteaddress
pub fn migration_address(address: Ed25519Address) -> String {
    ...
    let trytes = b1t6::encode::<T1B1Buf>(&[address.as_ref(), &result[0..4]].concat())
        .iter_trytes()
        .map(char::from)
        .collect::<String>();
    ...
    // Hash address to get the checksum
    let mut kerl = Kerl::new();
    let kerl_hash = kerl
        .digest(
            Trits::try_from_raw(
                &[tryte_address.to_inner().as_i8_slice(), &[0, 0, 0]].concat(),
                243,
            )
            .unwrap(),
        )
        ...
        .collect::<String>();
    // Add checksum
    format!("{}{}", transfer_address, &kerl_hash[72..81])
}
...
fn test_migration_address() {
    ...
    let migration_address = migration_address(address);
