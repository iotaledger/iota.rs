...
pub fn is_valid_transfer(transfer: &Transfer) -> bool {
    ...
    if !is_trytes(
            ...
            transfer.message().clone().len(),
        ) {
        return false;
    }
    if !is_trytes(
            &transfer.tag().clone(),
            transfer.tag().clone().len(),
        ) {
        return false;
    }
    true
...
mod tests {
    ...
    const TEST_HASH: &str = "OAATQS9VQLSXCLDJVJJVYUGONXAXOFMJOZNSYWRZSWECMXAQQURHQBJNLD9IOFEPGZEPEMPXCIVRX9999";
    ...
    #[test]
    fn test_is_trytes() {
        assert!(is_trytes(TEST_TRYTES, TEST_TRYTES.len()))
    }
    ...
    fn test_is_array_of_hashes() {
        assert!(is_array_of_hashes(&vec![TEST_HASH.to_string(), TEST_HASH.to_string()]));
    }

    #[test]
    fn test_is_array_of_trytes() {
        assert!(is_array_of_trytes(&vec![TEST_TRYTES.to_string(), TEST_TRYTES.to_string()]));
    }

    #[test]
    fn test_is_nine_trytes() {
        assert!(is_nine_trytes("999999999", 9));
    }
