...
mod tests {
    ...
    const TEST_SEED: &str = "IHDEENZYITYVYSPKAURUZAQKGVJEREFDJMYTANNXXGPZ9GJWTEOJJ9IPMXOGZNQLSNMFDSQOTZAEETUEA";
    ...
    fn test_long_seed_key_generation() {
        let seed = "EV9QRJFJZVFNLYUFXWKXMCRRPNAZYQVEYB9VEPUHQNXJCWKZFVUCTQJFCUAMXAHMMIUQUJDG9UGGQBPIY";

        for i in 1..5 {
            let key1 = key(&converter::trits_from_string(seed), 0, i);
            assert_eq!(KEY_LENGTH * i, key1.len());
            let key2 = key(&converter::trits_from_string(&(seed.to_string() + seed)), 0, i);
            assert_eq!(KEY_LENGTH * i, key2.len());
            let key3 = key(&converter::trits_from_string(&(seed.to_string() + seed + seed)), 0, i);
    ...
    #[test]
    fn test_verifying() {
        assert!(validate_signatures(&remove_checksum(ADDR), &vec![SIG1.to_string(), SIG2.to_string()], &remove_checksum(FIRST_ADDR)));
    }
}