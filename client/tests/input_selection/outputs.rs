...
fn no_inputs_provided() {
    ...
    let res = InputSelection::new(inputs, outputs, protocol_parameters).select();

    assert!(matches!(res, Err(Error::NoInputsProvided)))
}
...
fn no_outputs_provided() {
    ...
    assert!(matches!(res, Err(Error::NoOutputsProvided)))
}
